...
var transitionGallery = {
		...
		init : function() {
			var transitionsRecords = new Array();
			
			jQuery(".slider_tab > li").each(function(index , value ) {
				if(jQuery(this).hasClass("ui-state-active")){
					var tabHref = jQuery(this).find("a").attr("href").split("-");
					transitionId = tabHref[1];
				}
			});
			var tableId = "#ls-transition-gallery-"+transitionId;
			jQuery(tableId).empty();
			// Generate transition gallery
			transitionGallery.appendTransition('2D transitions', layerSliderTransitions['t2d'], '2d transitions_twod' , transitionId , tableId );
			transitionGallery.appendTransition('3D transitions', layerSliderTransitions['t3d'], '3d transitions_threed' , transitionId , tableId );

			// Prevent click events on transitions
			jQuery(document).on('click', tableId+' a', function(e) {
				var selectedclass = jQuery(this).attr('class');
				//alert(selectedclass);
				
				var selectedtransitions = jQuery(this).attr('rel').replace("tr" , "" );
				if ( selectedclass == '2d' ){
					var sliderId = jQuery("#transition2d_"+transitionId);
				} else {
					var sliderId = jQuery("#transition3d_"+transitionId);
				}
				
				if( sliderId.val().length > 0 ){
					transitionsRecords = sliderId.val().split(",");
				}
				if( jQuery(this).parent().hasClass("added") ){
					var transitions = new Array();
					jQuery(this).parent().removeClass("added");
					var j = 0;
					for( var i=0 ; i < transitionsRecords.length ; i++ ){
						if( transitionsRecords[i] != selectedtransitions && transitionsRecords[i] != "" ){
							//alert( transitionsRecords[i] +'<===>'+ selectedtransitions );
							/*transitions[j] = transitionsRecords[i];
							j++;*/
							transitions.push(transitionsRecords[i]);
						}
					}
					//alert(transitions.join());
					sliderId.val(transitions.join());
					return false;
				} else {
					//alert("123213213");
					transitionsRecords.push(selectedtransitions)
					var uniqueID = [];
					jQuery.each( transitionsRecords, function(i, el){
					    if(jQuery.inArray(el, uniqueID) === -1) uniqueID.push(el);
					});

					jQuery(this).parent().removeClass("added").addClass("added");
					sliderId.val(uniqueID.join());
					return false;
				}
			...
			// Show transitions
			jQuery(document).on('mouseenter', tableId+' a', function() {
				transitionGallery.showPreview(this);
			});

			// Hide transitions
			jQuery(document).on('mouseleave', tableId+' a', function() {
				transitionGallery.hidePreview();
			});
		},

		appendTransition : function(title, transitions, classes , transitionId , tableId ) {
			//alert(tableId);
			var sliderId2d = jQuery("#transition2d_"+transitionId);
			var sliderId3d = jQuery("#transition3d_"+transitionId);
			
			// Create transition group header
			jQuery(tableId+'').append( jQuery('<tbody>', { 'class' : classes })
					.append( jQuery('<tr>')
			...
			var selectedValues2d = sliderId2d.val().split(",");
			var selectedValues3d = sliderId3d.val().split(",");
			...
			for(c = 0; c < transitions.length; c+=2) {
				...
				var evenClass = "";
				var oddClass = "";
				itemOdd = itemOdd.toString();
				itemEven = itemEven.toString(); 
			
				
				/*if ( classes == '2d'  ){
					if ( (itemOdd % 2 != 0 && selectedValues2d.indexOf(itemOdd) != -1) &&  (itemEven % 2 == 0 && selectedValues2d.indexOf(itemEven) != -1) ){
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') .append( jQuery('<td class="added">')) .append( jQuery('<td class="added">')) );
					}
					else if( itemOdd % 2 != 0 && selectedValues2d.indexOf(itemOdd) != -1 ){
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') .append( jQuery('<td class="added">')) .append( jQuery('<td class="">')) );
					}else if( itemEven % 2 == 0 && selectedValues2d.indexOf(itemEven) != -1 ){
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') .append( jQuery('<td class="">')) .append( jQuery('<td  class="added">')) );
					}else {
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') .append( jQuery('<td class="">')) .append( jQuery('<td  class="">')) );
					}
					...
					if ( (itemOdd % 2 != 0 && selectedValues3d.indexOf(itemOdd) != -1) &&  (itemEven % 2 == 0 && selectedValues3d.indexOf(itemEven) != -1) ){
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') .append( jQuery('<td class="added">')) .append( jQuery('<td class="added">')) );
					}
					else if( itemOdd % 2 != 0 && selectedValues3d.indexOf(itemOdd) != -1 ){
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') .append( jQuery('<td class="added">')) .append( jQuery('<td class="">')) );
					}else if( itemEven % 2 == 0 && selectedValues3d.indexOf(itemEven) != -1 ){
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') .append( jQuery('<td class="">')) .append( jQuery('<td  class="added">')) );
					}else {
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') .append( jQuery('<td class="">')) .append( jQuery('<td  class="">')) );
					}
				...
				if ( classes == '2d'  ){
					if ( (itemOdd % 2 != 0 && selectedValues2d.indexOf(itemOdd) != -1) &&  (itemEven % 2 == 0 && selectedValues2d.indexOf(itemEven) != -1) ){
						jQuery(tableId+' tbody:last').append( jQuery('<tr>') 
								.append( jQuery('<td class="added">')) 
								.append( jQuery('<td class="added">')) );
					}
					else if( itemOdd % 2 != 0 && selectedValues2d.indexOf(itemOdd) != -1 ){
				...
				// Get new table row
				var tds = jQuery(tableId+' tr:last td');
				// Set first transition
				tds.eq(0).append( jQuery('<a>', { 'href' : '#', 'class' : classes , 'rel' : 'tr'+(c+1)+'', 'html' : ''+(c+1)+'. '+transitions[c]['name'] }))

				// Set second transition
				if(typeof transitions[(c+1)] != "undefined") {
					tds.eq(1).append( jQuery('<a>', { 'href' : '#','class' : classes , 'rel' : 'tr'+(c+2)+'', 'html' : ''+(c+2)+'. '+ transitions[(c+1)]['name'] }))
				}
			}
		},
		...
		showPreview : function(el) {
			...
			popup.find('.inner').transitionGallery({
				...
				transition : trObj,
				delay : 1500,
				path : '../typo3conf/ext/pits_layerslider/Resources/Public/img/'
			});
		},
